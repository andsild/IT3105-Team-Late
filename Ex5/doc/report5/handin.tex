...
\begin{enumerate}
    ...
    \item ``computer'': For each possible tile placement:
        \begin{enumerate}
            \item place a tile, and call all possible player moves,
                decrementing depth by 1
        \end{enumerate}
    \item \dotso continue until depth == 0
\end{enumerate}

The chance nodes discussed earlier are made implictly by stacking
...
\begin{tikzpicture}[->,>=stealth',level/.style={sibling distance = 4cm/#1,
  ...
  child{node [arn_r,label={[label distance=0.4cm]20:possible tiles\dotso{}}] {$15\cdot{p_{a}}$}
            ...
            child{node [arn_n] {30} edge from parent node[above left] {W} }
            % child{ node [arn_n] {10}
...
;
\end{tikzpicture}
\caption{Example expectimax tree}
\end{figure}
